...
var app = angular.module("app.todos", ["xeditable", "ngSanitize"]);

app.run(function (editableOptions) {
    editableOptions.theme = 'bs4';
});
app.controller("todoController", ['$scope', 'svTodos', '$filter', '$sce', function ($scope, svTodos, $filter, $sce) {
    // console.log(document.getElementById('user_name').value,document.getElementById('user_id').value,document.getElementById('user_track').value)
    const URLserver = document.domain.indexOf('localhost') >= 0 ? location.protocol + "//" + document.domain + ":" + location.port : location.protocol + "//" + document.domain;
    const socket = io.connect(URLserver); //http://localhost:3000
    socket.on("connect", function () {
        //ask name
        let username = document.getElementById('user_name').value;
        let id_user = document.getElementById('user_id').value;
        let trackper = document.getElementById('user_track').value;
    ...
    $scope.formData = [];
    $scope.formData.user_name = "Tất cả";
    $scope.loading = true;
    ...
    if (window.Notification) {
        ...
        $scope.valnotifi = " (Không Thông báo)";
    };
    svTodos.get("/nodetodo").then(function (data) {
        ...
        //console.log('data.data.listusers: ',data.data.listusers)
        for (var i = 0; i < $scope.todos.length; i++) {
            ...
            } else if (datadate.getTime() - now.getTime() <= delaydate && datadate.getTime() - now.getTime() > 0) {
        ...
        //console.log('messdatas: ',messdatas);
        // console.log(data.data.user);
        //socket login
        for (var i = 0; i < data.data.datachat.length; i++) {
            data.data.datachat[i].created_at = new Date(data.data.datachat[i].created_at);
        ...
        if (window.Notification) {
            Notification.requestPermission().then(function (result) {
                ...
                if (result === 'granted') {
                    $scope.valnotifi = "";
                    $scope.$digest();
                    const beamsClient = new PusherPushNotifications.Client({ instanceId: '2131fdf4-c465-4976-8987-018f733f6fc8', });
                    beamsClient.start()
                        .then(() => beamsClient.setDeviceInterests([idNotif + "notify"]))
    ...
    socket.on("update_message", function (datachat) {
        ...
        // console.log(datachat.msg)
        datachat.msg = $sce.trustAsHtml(datachat.msg)
    ...
    $scope.filterTodo = function () {
        ...
        if ($scope.formData.trackper <= 2) {
            ...
            //console.log('$scope.formData.id_user: ', $scope.formData.id_user)
            if ($scope.formData.id_user == "" || $scope.formData.id_user == undefined || $scope.formData.id_user == null) {
                messdatas.re_user = 0;
            } else {
            ...
            for (let j = 0; j < listusers.length; j++) {
                if (($scope.formData.id_user) == (listusers[j].id)) {
                    $scope.formData.re_user_trackper = listusers[j].trackper
                    break;
                };
            };
            $scope.countcurrent = 0;
            ...
            if (messdatas.re_user != 51296 && messdatas.re_user != 0) {
                ...
                for (var i = 0; i < arrfilter.length; i++) {
                    ...
                    if (Number(arrfilter[i].id_user) === Number($scope.formData.id_user)) {
                        $scope.todos.push(arrfilter[i]);
                        if (arrfilter[i].end_at < now && arrfilter[i].isDone == 0) {
                            $scope.countlate++
                        };
    ...
    $scope.createTodo = function () {
        $scope.loading = true;
        //console.log(todo);
        if (!$scope.formData.text_job || !$scope.formData.end_at || !$scope.formData.user_name || $scope.formData.id_user == "51296" || $scope.formData.id_user == "") {
            ...
            var todo = {
                ...
                // text_jobdemo: $scope.formData.text_job,
    ...
    $scope.updateTodo = function (todo) {
        ...
        $scope.loading = true;
        if (todo.text_jobdemo && todo.text_jobdemo.length > 1000) {
            $scope.loading = false;
        ...
        if ($scope.formData.trackper == 3) {
            ...
            svTodos.update(todo).then(function (data) {
                ...
                if (nowdayb.getTime() - now.getTime() > delaydate) {
                    todo.timeline = 1;
                } else if (nowdayb.getTime() - now.getTime() <= delaydate && nowdayb.getTime() - now.getTime() > 0) {
                    todo.timeline = 2;
                } else {
                    todo.timeline = 3;
                };
    ...
    $scope.updateTodoFN = function (todo) {
        ...
        svTodos.updateFn(params).then(function (data) {
    ...
    $scope.deleteTodo = function (todo) {
        ...
        //alert(delId);
        //console.log(todo);
        svTodos.delete(delId).then(function (data) {
            //console.log(todo);
